...
export default function OfflineMembers() {
  ...
  const handleExport = async () => {
    try {
      const response = await axiosInstance.get(`/api/admin/gym-stats/${gymId}/offline-members`, {
        params: {
          export: true,
      ...
      link.href = url;
      link.setAttribute('download', `offline-members-${gymName || gymId}-${new Date().toISOString().split('T')[0]}.xlsx`);
